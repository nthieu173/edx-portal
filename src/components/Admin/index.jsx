...
class Admin extends React.Component {
  ...
  getTableDataForAction(actionSlug) {
    const defaultData = {
      title: 'Full Report',
      component: <CourseEnrollmentsTable />,
    };
    ...
    return (
      <Link to={path} className="reset btn btn-sm btn-outline-primary ml-3">
        <span>
          ...
          Reset to {this.getTableDataForAction().title}
        </span>
      </Link>
    );
  }
  ...
  render() {
    ...
    const { params: { slug } } = match;
    const tableData = this.getTableDataForAction(slug);
